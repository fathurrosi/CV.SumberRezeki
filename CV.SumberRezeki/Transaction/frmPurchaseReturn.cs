...
{
    public partial class frmPurchaseReturn : MasterPage, IMasterHeader, ITransButton
    {
        ...
        {
            List<Variable> list = PurchaseItem.GetAllPurchaseCounter();
            ...
            PurchaseReturn item = PurchaseReturnItem.GetByCode(PurchaseReturnNo);
            if (item != null)
            {
        ...
        void LoadDetail(List<PurchaseReturnDetail> list)
        {
            //List<PurchaseReturnDetail> _list = new List<PurchaseReturnDetail>();
            //decimal total = 0;
            //if (list != null)
            //{
        ...
        public bool IsValid()
        {
            try
            {
                List<PurchaseReturnDetail> details = (List<PurchaseReturnDetail>)grid.DataSource;
        ...
        public void Add()
        {
            ...
            List<PurchaseReturnDetail> list = (List<PurchaseReturnDetail>)grid.DataSource;
            frmPurchaseReturnDetail fb = new frmPurchaseReturnDetail();
            fb.PurchaseCounter = counter;
            if (fb.ShowDialog() == System.Windows.Forms.DialogResult.OK)
            {
                if (list == null)
                {
                    list = new List<PurchaseReturnDetail>();
                }
        ...
        public void Edit()
        {
            ...
            List<PurchaseReturnDetail> list = (List<PurchaseReturnDetail>)grid.DataSource;
            PurchaseReturnDetail selected = list.Where(t => string.Format("{0}", t.UniqueID) == uniqueID).FirstOrDefault();
            frmPurchaseReturnDetail fb = new frmPurchaseReturnDetail();
        ...
        public void Delete()
        {
            ...
            if (dialogResult == System.Windows.Forms.DialogResult.Yes)
            {
                ...
                List<PurchaseReturnDetail> list = (List<PurchaseReturnDetail>)grid.DataSource;
                if (list == null) list = new List<PurchaseReturnDetail>();
                else
                {
                    PurchaseReturnDetail selected = list.Where(t => string.Format("{0}", t.UniqueID) == code).FirstOrDefault();
                    if (selected != null)
                    {
        ...
        private void LoadPurchase(string strukNo)
        {
            int counter = 0;
            int.TryParse(strukNo, out counter);
            Purchase item = PurchaseItem.GetByCounter(counter);
            if (item == null)
            {
                Utilities.ShowInformation(string.Format("Tidak ada transaksi dengan no struk : {0}", strukNo));
                return;
            }
            txtPurchaseNo.Text = item.PurchaseNo;
            txtPurchaseDate.Text = string.Format("{0:dd MMMM yyyy}", item.PurchaseDate);

            Supplier customer = SupplierItem.GetByCode(item.SupplierCode);
            if (customer != null)
            {
            ...
            List<PurchaseReturnDetail> list = new List<PurchaseReturnDetail>();// item.Details;
            LoadDetail(list);
        }
        ...
        public void Save()
        {
            ...
            if (item == null)
            {
                ...
                int index = PurchaseReturnItem.GetIndexPerdate(item.ReturnDate);
                item.ReturnNo = string.Format("{0}-{1}", item.ReturnDate.ToString(Utilities.FORMAT_DateTime_Flat), index);
                item.TransactionID = txtPurchaseNo.Text;
                item.Supplier = CustomerCode;
            ...
            int result = PurchaseReturnItem.Insert(item);
            if (result > 0)
                DialogResult = System.Windows.Forms.DialogResult.OK;
        }
