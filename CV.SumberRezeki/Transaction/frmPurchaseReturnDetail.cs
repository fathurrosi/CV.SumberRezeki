...
{
    public partial class frmPurchaseReturnDetail : MasterPage, ITransButton
    {
        public int PurchaseCounter { get; set; }
        public string ReturnNo { get; set; }
        public frmPurchaseReturnDetail()
        {
            InitializeComponent();
        ...
        public void Save()
        {
            ...
            try
            {
                if (Detail == null)
                {
                    _Detail = new PurchaseReturnDetail();
                ...
                Purchase sItem = PurchaseItem.GetByCounter(PurchaseCounter);
                if (sItem == null)
                {
                    sItem.Details = new List<PurchaseDetail>();
                }



                PurchaseDetail detailOnPurchases = sItem.Details.Where(t => t.CatalogID == CatalogId).FirstOrDefault();
                if (detailOnPurchases == null)
                {
                    Utilities.ShowValidation("Barang harus dipilih");
                    return;
                }
                ...
                //frmQty.lblQty.Text = string.Format("Masukan Jumlah Untuk Produk Ini (Max - {0}) :", Utilities.ToString(detailOnPurchases.Quantity, "N2"));
                frmQty.MaxQty = detailOnPurchases.Qty;
                frmQty.MaxColly = detailOnPurchases.Coli;
                ...
                _Detail.Item = item;
                _Detail.Price = detailOnPurchases.PricePerUnit;
                _Detail.TotalPrice = _Detail.Qty * _Detail.Price;
                if (item != null)
                {
                    _Detail.CatalogName = item.Name;
                    _Detail.Catalog = item.ID.ToString();
                    _Detail.Unit = detailOnPurchases.Unit;
                    //_Detail.UnitName = detailOnPurchases.UnitName;
        ...
        void LoadGrid(string text)
        {
            ...
            List<PurchaseDetail> list = sItem.Details;
            grid.DataSource = (from t in list
                               select new { CatalogID = t.CatalogID, Name = t.CatalogName, t.Unit, Quantity = t.Qty, Price = t.PricePerUnit, t.Coli })
