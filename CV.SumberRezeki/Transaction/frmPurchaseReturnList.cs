...
{
    public partial class frmPurchaseReturnList : MasterPage, IMasterHeader, IMasterFooter
    {
        ...
        {
            List<PurchaseReturn> list = PurchaseReturnItem.GetCstmPaging(text, offset, pageSize);
            grid.AutoGenerateColumns = false;
            grid.DataSource = list;
            ctlFooter1.TotalRows = PurchaseReturnItem.GetCstmRecordCount(text);
        }
        ...
        public void Add()
        {
            frmPurchaseReturn f = new frmPurchaseReturn();
            f.Text = this.Text;
            f.MaximizeBox = false;
            f.MinimizeBox = false;
            f.ControlBox = false;
            f.FormBorderStyle = System.Windows.Forms.FormBorderStyle.FixedDialog;
            f.PurchaseReturnNo = string.Empty;
            f.ShowDialog();
            Search();
        }
        ...
        public void Edit()
        {
            ...
            f.PurchaseReturnNo = string.Format("{0}", grid["colReturnNo", Row].Value);
            f.IsView = true;
        ...
        public void Delete()
        {
            ...
            if (dialogResult == System.Windows.Forms.DialogResult.Yes)
            {
                ...
                PurchaseReturn item = PurchaseReturnItem.GetByCode(returnNo);

                int result = PurchaseReturnItem.Delete(returnNo);
                if (result >0)
                {
